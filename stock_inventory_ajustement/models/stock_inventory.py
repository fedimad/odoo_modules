...
class InventoryAdjustmentsGroup(models.Model):
    ...
    def _compute_count_stock_quants(self):
        for rec in self:
            ...
            _logger.warning(' \n\n _compute_count_stock_quants ************** quants_to_do =  %s \n\n' %(quants_to_do))
    ...
    def action_state_to_draft(self):
        self.ensure_one()
        self.state = "draft"
        _logger.warning('action_state_to_draft ************** self.stock_quant_ids =====  %s' %(self.stock_quant_ids))
        self.stock_quant_ids.filtered(lambda q: q.current_inventory_id.id == self.id).action_set_inventory_quantity_to_zero()
        self.stock_quant_ids.filtered(
            lambda q: q.current_inventory_id.id == self.id
        ).update({
                "to_do": False,
    ...
    def action_view_stock_moves(self):
        ...
        result = self.env["ir.actions.act_window"]._for_xml_id(
            "stock_inventory_adjustment.action_view_stock_move_line_inventory_tree"
        )
...
class InventoryAdjustmentsLine(models.Model):
    ...
    prod_lot_id = fields.Many2one(
        'stock.production.lot', 'Lot/Numéro de série', check_company=True,
